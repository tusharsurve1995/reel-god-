...
* **Audio Voice Terminal Chat**: Restored the text chat box input and 'Send' button on the mobile UI by registering event listeners in `app.js` mapping to the backend `/api/voice-chat` endpoint. The agent now returns text replies and synthesizes them aloud via Text-to-Speech (TTS), allowing full conversational control over the application from your phone.
...
* **Local Socket.IO & Fallback Registry**: Changed the Socket.IO library import in `index.html` from an external CDN to the local `/socket.io/socket.io.js` endpoint served directly by Flask-SocketIO. This resolves network blocks (e.g. ad blockers or firewalls) that cause `io is not defined` errors. Wrapped Socket.IO setup in a safe initialization fallback so that other JS handlers (like text chat REST requests) continue to function even if Socket.IO fails to load.
* **AudioContext Browser Sandbox Security Fix**: Moved the initialization of `audioContext` and `playbackContext` to run synchronously inside the `click` listener of `#btn-live-talk`. This conforms to modern mobile and desktop browser security policies, which suspend/block audio instantiation when called asynchronously after an `await fetch` call.
* **Microphone Race Condition Fix**: Added a safety check in `startMicRecording()` to check `!isLiveActive || !audioContext` before processing audio. This prevents a `TypeError` crash if the WebSocket connection is closed/rejected before the asynchronous `getUserMedia()` mic permission promise resolves.
